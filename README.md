...
> ⚠️ **Важно:** В Caddy для корректной раздачи отдельного статического файла на фоне работающего прокси необходимо обернуть раздачу файла в блок handle /routing.json {...}, иначе reverse_proxy перехватит этот запрос.
